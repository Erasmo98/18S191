...
function extend(v::AbstractVector, i::Integer)
	if i < 1
		return v[1]
	elseif  i > (n = length(v))
		return v[n]
...
# ╔═╡ 77a39c29-ef89-41ea-b0c5-c559601d380d
extend(v::AbstractVector, is) = [extend(v, i) for i in is]
...
# ╔═╡ c8623987-863d-4424-a489-24030842514e
box_blur(v::AbstractArray, l) = [box_blur(v, l, i) for i in 1:length(v)]
...
# ╔═╡ 8d94d392-dcd9-43f7-8782-1fb0928fe3f8
convolve(v::AbstractVector, k) = (i -> convolve(v, k, i)).(1:length(v))
# convolve(v::AbstractVector, k) = [convolve(v, k, i) for i in 1:length(v)]
...
function extend(M::AbstractMatrix, i::Integer, j::Integer)
	m, n = size(M)
	return M[clamp(i, 1, m), clamp(j, 1, n)]
end

# ╔═╡ 1e18c351-e68d-41f2-b988-1722de379248
extend(M::AbstractMatrix, is, js) = [extend(M, i, j) for i in is, j in js]
...
function box_blur(v::AbstractArray, l, i)
	return mean( extend(v, i-l:i+l) )
end
...
function convolve(v::AbstractVector, k, i)
	l = ( length(k) - 1 ) ÷ 2
	return sum(k .* extend(v, i-l:i+l))
end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix, i, j)
	# K is of size (2m+1)×(2n+1)
	m, n = (size(K) .- 1) .÷ 2 
	return sum( K .* extend(M, i-m:i+m, j-n:j+n) )
end

# ╔═╡ d2c71ca8-495e-497a-b790-0a8cd60bc654
convolve(M::AbstractMatrix, K::AbstractMatrix) = [convolve(M, K, i, j) for i in 1:size(M, 1), j in 1:size(M, 2)]
...
# ╔═╡ 1268112b-8436-47d8-a130-46794dea3f9d
rgb2tuple(pixel::AbstractRGB) = (pixel.r, pixel.g, pixel.b)

# ╔═╡ d5e90b80-2fea-4478-b0c4-68e126103d22
begin
	import Base: sqrt, ^
	sqrt(pixel::AbstractRGB) = typeof(pixel)(sqrt.(rgb2tuple(pixel))...)
	^(pixel::AbstractRGB, α) = typeof(pixel)(rgb2tuple(pixel).^α...)
end
...
# ╔═╡ 688a43ff-a2f7-4f5a-b29a-d4dbb20fe330
function gaussian_kernel_2D(l; σ = 1)
	K = gauss.(-l:l, σ)
	K = K * permutedims(K)
	return K / sum(K)
end
...
function with_sobel_edge_detect(image)
	...
	return sqrt.(convolve(image, G).^2 + convolve(image, permutedims(G)).^2)
end
