...
begin
	...
	for i in 1:27
		if sample_freq_matrix[i, i] != 0
			push!(double_letters, alphabet[i])
		end
	end
end

# ╔═╡ cfd25cf9-f1a5-4e67-b0b4-dc8cc0c5327c
...
row_col_answer = md""" The sum of the i-th row is the probability of finding the i-th letter of the alphabet as the second the second letter y a pair of letter while the sum of the i-th column is the probability of finding the same letter as the firt one in the pair. """
...
function completion_cache(grams)
	...
	for gram in grams
		...
		if haskey(cache, key)
			cache[key] = push!(cache[key], gram[end])
		else
			cache[key] = [gram[end]]
		end
	end
